...
def main():
    ...
    BASE_DIR = "data/Toys"
    TIGER_DIR = "/workspace/user_code/ED-RQVAE/edrqvae_data/Toys/v2"
    #TIGER_DIR = "/workspace/user_code/baseline/RQ-VAE/rqvae_data/Toys"
    #TIGER_DIR = "/workspace/user_code/baseline/varlen_semantic_ids/result/Toys/varlen"
    ...
    best_model_path = os.path.join(save_dir, "tiger_best_v3_16.pt")
    #best_model_path = os.path.join(save_dir, "tiger_best_rq.pt")
    #best_model_path = os.path.join(save_dir, "tiger_best_v3_16.pt")
    
    log_file_path = "/workspace/user_code/baseline/RQ-VAE/logs/Toys/ed-rqvae/test_v3_16.txt"
    #log_file_path = "/workspace/user_code/baseline/RQ-VAE/logs/Toys/rqvae/train_log_16.txt"
    #log_file_path = "/workspace/user_code/baseline/RQ-VAE/logs/Toys/varlen/test_v3_16.txt"
    ...
    model = Tiger(
        ...
        d_model=512, nhead=8, num_layers=16, dropout=0.15
    )
    ...
    # ==========================================
    # 7. Compute metrics (完美 Item ID 展开与分箱 + 🌟 Max-per-Prefix)
    # ==========================================
    if accelerator.is_main_process:
        print("\n[*] 正在构建 SID 到 Item ID 的倒排索引...")
        
        # 🌟 核心截断超参数 
        # 你可以调节这两个值来寻找最优解，建议先试 PREFIX_LEN=3, MAX_PER_PREFIX=2  #1(3,5) #2(4,2)
        PREFIX_LEN = 4       # 定义“簇”的层级（3表示控制到 L3 级别的前缀）
        MAX_PER_PREFIX = 2   # 同一个前缀簇下，最多允许进入 Top-50 的商品数量 2 3 5
        ...
        print(f"\n[*] 开始执行 Item 级别的 Beam Search 展开 (固定 L3 降级重排，上限 {MAX_PER_PREFIX} 个)...")
        final_preds = []
        final_gts = []
        ...
        for i in range(len(final_users_list)):
            ...
            for beam_idx in range(EVAL_BEAM):
                ...
                current_prefix = pred_sid[:PREFIX_LEN]
                ...
                for item in mapped_items:
                    ...
                    # 判断当前前缀是否已经满员
                    if prefix_counts.get(current_prefix, 0) < MAX_PER_PREFIX:
            ...
            # --- 第二遍：如果多样性商品凑不够 50 个，启动备胎池兜底！ ---
            if len(pred_items) < MAX_K:
                needed = MAX_K - len(pred_items)
                pred_items.extend(fallback_items[:needed])
